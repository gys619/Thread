...
[task_local]
#冰雪闪耀季
30 0,2 7-24 12 * jd_wish3.js, tag=冰雪闪耀季, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
      ...
      for (let v = 0; v < appIdArr.length; v++) {
        ...
        for (let j = 0; j < $.shareCode.length && $.canHelp; j++) {
          if ($.shareCode[j].appId === appId) {
            ...
            $.delcode = false
            await harmony_collectScore({ "appId": appId, "taskToken": $.shareCode[j].code, "actionType": "0", "taskId": "5" })
            await $.wait(2000)
            if ($.delcode) {
