...
SEVENDAY_LIST   // https://lzkj-isv.isvjcloud.com/sign/sevenDay/signActivity?activityId=<活动id>
SEVENDAY_LIST2  // https://lzkj-isv.isvjcloud.com/sign/signActivity2?activityId=<活动id>
SEVENDAY_LIST3  // https://cjhy-isv.isvjcloud.com/sign/signActivity?activityId=<活动id>
...
async function signActivity() {
    ...
    if ($.token) {
        await getMyPing();
        if ($.secretPin) {
            await task('sign/wx/getShopInfo', `venderId=${$.venderId}`, 1);
            await task('sign/sevenDay/wx/getSignInfo', `actId=${$.activityId}&venderId=${$.venderId}&pin=${encodeURIComponent($.secretPin)}`, 1);
...
async function signActivity2() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            await task('sign/wx/getShopInfo', `venderId=${$.venderId}`, 1);
            await task('sign/wx/getActivity', `actId=${$.activityId}&venderId=${$.venderId}`, 1);
            await task('common/accessLogWithAD', `venderId=${$.venderId}&code=${$.activityType}&pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&pageUrl=${$.activityUrl}&subType=app&adSource=tg_xuanFuTuBiao`, 1);
        } else {
            $.log("没有成功获取到用户信息")
        }
...
async function signActivity3() {
    ...
    if ($.token) {
        await getMyPing2();
        if ($.secretPin) {
            await task2('sign/wx/getShopInfo', `venderId=${$.venderId}`, 1);
            await task2('sign/wx/getActivity', `actId=${$.activityId}&venderId=${$.venderId}`, 1);
...
async function task(function_id, body, isCommon = 0) {
                        ...
                        if (data) {
                            switch (function_id) {
                                case 'customer/getSimpleActInfoVo':
                                    ...
                                    break;
                                case 'sign/wx/getShopInfo':
                                    if (data.isOk) {
                                        console.log('❖ ' + data.shopInfo.shopName);
                                    }
                                    break;
                                case 'sign/sevenDay/wx/getSignInfo':
                                    if (data) {
                                        // console.log(data);
                                        if (data.isOk) {
                                            prize_name = '';
                                            giftConditions = data.giftConditions;
                                            isSign = data.isSign;
                                            actRule = data.actRule;
                                            actRule = actRule.split('\n')[0].split('：')[1].split('；')[0].replace(' - ', ' 至 ');

                                            if (isSign == "y") {
                                                console.log("活动状态：已经结束");
                                            } else if (isSign == "n") {
                                                console.log("活动状态：进行中～");
                                            }
                                            console.log('活动时间：' + actRule);
                                            console.log(`店铺链接：https://shop.m.jd.com/?venderId=${$.venderId}`);
                                            console.log(`活动链接：https://lzkj-isv.isvjcloud.com/sign/sevenDay/signActivity?activityId=${$.activityId}`);

                                            for (let i = 0; i < giftConditions.length; i++) {
                                                giftName = giftConditions[i].gift['giftName']

                                                var exits_status = judgePrize(giftName);
                                                if (exits_status) continue;

                                                prize_name += giftConditions[i].gift['giftName'] + ' ';
                                            }
                                            if (prize_name != '') {
                                                console.log(`活动奖品：${prize_name}`);
                                            } else {
                                                console.log(`活动奖品：垃圾`);
                                            }
                                        }
                                    ...
                                    break
                                case 'sign/wx/getActivity':
                                    if (data) {
                                        // console.log(data);
                                        if (data.isOk) {
                                            prize_name = '';
                                            giftConditions = data.act.wxSignActivityGiftBean.giftConditions;
                                            actTimeStr = data.act.actTimeStr
                                            endTime = data.act.endTime
                                            ...
                                            console.log(`店铺链接：https://shop.m.jd.com/?venderId=${$.venderId}`);
                                            console.log(`活动链接：https://lzkj-isv.isvjcloud.com/sign/signActivity2?activityId=${$.activityId}`);
...
async function task2(function_id, body, isCommon = 0) {
    return new Promise(resolve => {
                                ...
                                case 'sign/wx/getActivity':
                                    if (data) {
                                        ...
                                        if (data.isOk) {
                                            ...
                                            console.log('活动时间：' + actTimeStr);
                                            console.log(`店铺链接：https://shop.m.jd.com/?venderId=${$.venderId}`);
                                            console.log(`活动链接：https://cjhy-isv.isvjcloud.com/sign/signActivity?activityId=${$.activityId}`);
...
function judgePrize(name) {
    var flag = false;
    for (let e of lajiprizewords) {
        if (name.includes(e)) {
            flag = true;
            break
        }
