...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({
      url: `http://www.11111114/jdcodes/getcode.php?type=health&num=${randomCount}`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
...
function submitCode() {
    return new Promise(async resolve => {
    $.get({url: `http://www.11111114/jdcodes/submit.php?code=${myInviteCode}&type=health`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
