...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = [
                '4dc4d45620cf4d749b4786d0e4c1f8b5',
                '9d5fd90731e04454829bc6319828df63',
                '8caac0f902f445b187d8cdb2eaf1507f',
            ]
...
async function rush() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.openCardStatus) {
                $.log("模拟上报访问记录")
                for (let i = 0; i < ($.openCardStatus.cardList1.length + $.openCardStatus.cardList2.length + $.openCardStatus.cardList3.length); i++) {
                    await task('crm/pageVisit/insertCrmPageVisit', `venderId=1000005754&pageId=dz04bfe2a64b46bbcb1110fc0ca9bf&elementId=${encodeURIComponent(`去开卡${i}`)}&pin=${encodeURIComponent($.secretPin)}`, 1)
                    await $.wait(500)
                }
                t1TaskList = []
                ...
                t3TaskList = []
                $.openCardStatus.cardList3.filter((x) => { if (x.status === 0) { t3TaskList.push(x) } })
                if (t1TaskList.length < 1 && t2TaskList.length < 1 && t3TaskList.length < 1) {
                ...
                await task('dz/openCard/checkOpenCard', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&shareUuid=${$.authorCode}&pin=${encodeURIComponent($.secretPin)}`)
                await $.wait(2000)
                await task('dz/openCard/saveTask', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=2&taskValue=100016848153`)
                // await task('common/accessLogWithAD', `venderId=${$.activityShopId}&code=99&pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&pageUrl=${$.activityUrl}&subType=app&adSource=null`, 1);
                // await $.wait(2000)
