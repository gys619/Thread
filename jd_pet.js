...
async function jdPet() {
    try {
        ...
        if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
            ...
            // 报告运行次数
            $.get({
                    url: `https://cdn.11111112/api/runTimes?activityId=pet&sharecode=${$.petInfo.shareCode}`,
                    headers: {
                        'Host': 'api.11111117'
                    },
                    timeout: 10000
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({ url: `https://cdn.11111112/api/pet/${randomCount}`, headers: { 'Host': 'api.11111117' }, timeout: 10000 }, (err, resp, data) => {
            try {
                if (err) {
