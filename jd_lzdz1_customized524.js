...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = [
        '8bb3954e8c2445c7a8fbcd05c5ab8f74',
        '9c2945adfd1c41d6b9961be26f3a8c9a',
      ...
      $.activityShopId = "1000003663";
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/bigname/memberday/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=1000003663&lng=00.000000&lat=00.000000&sid=&un_area=`;
      await member();
...
async function member() {
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      if ($.index === 1) {
        await task("bigname/memberday/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1);
      } else {
        await task("bigname/memberday/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`);
      }
      // $.log("关注店铺");
      // await task("bigname/memberday/saveTask", `activityId=${$.activityId}&actorUuid=${encodeURIComponent($.actorUuid)}&pin=${encodeURIComponent($.secretPin)}&taskType=23&taskValue=23&shareUuid=${encodeURIComponent($.authorCode)}`);
      // await task("taskact/common/drawContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
      await task("bigname/memberday/initOpenCard", `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}`);
      $.log("加入店铺会员");
      if ($.openCardList) {
        taskList = [];
      ...
      // await task('bigname/memberday/startGame', `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}`)
      // await task('bigname/memberday/endGame', `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&gameId=${$.gameId}&score=${$.score}`)
      // console.log('任务 -> ')
      // await $.wait(2000)
