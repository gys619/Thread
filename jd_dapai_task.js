...
let activityCookie = ''
//此处修改助力码
let helpnum = 'ZjZaq7+wrehqofWNRvE3hGBUh0gimxufjFRMgNwy0cYiU+N6ZQy0LI2KZrs00RAY'
let DAPAINEW = "";
let opencard_toShop = "true";
let opencard_shop = "30"
DAPAINEW = $.isNode() ? (process.env.DAPAINEW ? process.env.DAPAINEW : `${DAPAINEW}`) : ($.getdata('DAPAINEW') ? $.getdata('DAPAINEW') : `${DAPAINEW}`);
if (!DAPAINEW) {
  console.log(`\n请填写大牌大牌集合通用开卡的活动ID,变量是DAPAINEW\n`)
  return;
}!(async () => {
  console.log(`当前活动ID：` + DAPAINEW)
  console.log(`当前设定浏览次数：` + opencard_shop)
  if (!cookiesArr[0]) {
...
async function run() {
  try {
    ...
    if (opencard_toShop + "" == "true") {
      let count = opencard_shop
      //console.log(`默认浏览任务次数:${count}`)
      for (m = 1; count--; m++) {
        console.log(`开始第${m}次浏览`)
        $.missionType = 'viewShop' // 浏览店铺
        await takePostRequest('mission');
        await $.wait(parseInt(Math.random() * 2000 + 1000, 10))
        $.missionType = 'viewGoods' // 浏览商品
        await takePostRequest('mission');
        await $.wait(parseInt(Math.random() * 2000 + 1000, 10))
      }
    } else {
