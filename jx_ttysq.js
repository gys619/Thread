...
!(async () => {
    ...
    console.log(`互助码${JSON.stringify($.shareCoseList.length)}个\n`)
    //助力任务
    for (let i = 0; i < cookiesArr.length; i++) {
        $.canHelp = true
        cookie = cookiesArr[i];
...
function getAuthorShareCode(url='https://raw.githubusercontent.com/11111115/params/main/ttys.json') {
                ...
                if (err) {
                    ...
                    if (function_name === "jxnhj/GetUserInfo") {
                        if (data.iRet === 0) {
                            ...
                            if (show === true) {
                                console.log(`现在等级${$.level},年货值有${$.goodsNum},有${$.luckyNum}份红包，预计分得${$.luckyWorth}元,可抽奖次数${$.lotteryNum}`)
