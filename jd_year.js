...
async function main() {
    ...
    if ($.token) {
        ...
        if ($.accessToken) {
            ...
            if ($.tasklist) {
                $.log('点亮..')
                await taskPost('get_light_task', `uuid=${$.uuid}&source=shareFriend`, JSON.stringify({ "now_date": `${nowdate}` }));
                $.log('等待15s')
                await $.wait(15000)
                await taskPost('do_light_task', `uuid=${$.uuid}&source=shareFriend`, JSON.stringify({ "now_date": `${nowdate}` }));
                for (let vo of $.tasklist) {
                    if (vo.left === vo.right) {
                        console.log(`任务：${vo.title}，已完成`);
                        continue;
                    }
                    if (vo.id === 1) {
                        console.log(`去执行${vo.title}任务`);
...
function taskPost(function_id, body, body2) {
    return new Promise(async resolve => {
        $.post(taskPostUrl(function_id, body, body2), async (err, resp, data) => {
            try {
                if (data) {
                    data = JSON.parse(data);
                    if (data) {
                        switch (function_id) {
                            case 'get_light_task':
                                break;
                            case 'do_light_task':
                                console.log(`获得爆竹${data.add_coins},总爆竹:${data.coins}`)
                                break;
                            case 'jd-user-info':
...
function taskPostUrl(function_id, body, body2) {
    return {
        ...
        headers: {
            ...
            "Origin": "xinrui-isv.isvjcloud.com",
            "User-Agent": UA,
