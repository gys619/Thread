...
号1默认给我助力,后续接龙 2给1 3给2
 22.0复制整段话 Https:/Jt3uI8tUnEYKNg星系牧场养牛牛，可获得DHA专属奶！%N6beE2Em6a!后扌丁开乛倞崬
[task_local]
#星系牧场
1 0-23/2 * * * 
*/
...
message = ""
$.shareuuid = "85ecd51344fb4b38a892797036e30417"
    !(async () => {
        if (!cookiesArr[0]) {
        ...
        for (let i = 0; i <cookiesArr.length; i++) {
            ...
            if (cookie) {
                ...
                if ($.cando) {
                    ...
                    console.log(`可兑换 ${exchanges} 次 20京豆🐶`) 
                    for(q = 0;q<exchanges && Exchange;q++){
                    await exchange(13)    //16是100豆
                    }
                    await getinfo()                     
                    if(!Exchange){console.log("你 默认 不兑换东西,请自行进去活动兑换")}                    
                    message += `【京东账号${$.index}】${$.nickName || $.UserName}\n${$.cow} 兑换京豆🐶 ${$.exchange}  ${$.drawresult}\n\n`
                    if($.index%3===0) 
                   {
                  await $.wait(60*1000) 
                  console.log("休息休息~") 
