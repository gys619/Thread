...
!(async () => {
    ...
    var runCK = [];
    for(var i=0;i<cookiesArr.length;i+=5){
        runCK.push(cookiesArr.slice(i,i+5));
    }
...
async function main(cookie) {
    ...
    for (let i = 0; i < 3; i++) {
        await takeRequest(cookie,`party1031_time`,'{}');
        await $.wait(3000);
        let retInfo = await takeRequest(cookie,`party1031_lottery`,'{"filled":true}',true);
