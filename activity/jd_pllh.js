...
!(async () => {
    ...
    $.activityId = "52f4d41fb7514aea8777a09683e7e1a6"
    $.shareUuid = "3987e575d309492390cabbb026bab379"
    $.venderIds = "688693"
    console.log(`活动入口: https://lzdz-isv.isvjcloud.com/categoryUnion/activity?activityId=${$.activityId}&tplId=0003`)
    let shareUuidArr = ["7df4c8b26"]
...
async function run() {
    try {
        ...
        for (const vo of $.unionShopInfos) {
            if ($.runFalag == false) break
            $.venderIdss = vo.value
            $.visitSkuValue = $.venderIdss;
            await takePostRequest('关注');
            await $.wait(parseInt(Math.random() * 3000 + 500, 10))
        }
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'getMyPing':
            ...
            body = `userId=688693&token=${$.Token}&fromType=APP`;
            break;
        ...
        case 'activityContent':
            url = `${url}/categoryUnion/activityContent`;
            body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&pinImg=${encodeURIComponent($.attrTouXiang)}&nick=${encodeURIComponent($.nickname)}&shareUuid=${$.shareUuid}`
            break;
        case 'drawContent':
            url = `${url}/dingzhi/taskact/common/drawContent`;
            body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}`
            break;
        case '关注':
        case '加购':
            url = `${url}/wxActionCommon/followShop`;
            body = `activityId=${$.activityId}&buyerNick=${encodeURIComponent($.Pin)}&userId=${$.visitSkuValue}&actorUuid=${$.actorUuid}&activityType=99&shareUuid=${$.shareUuid}`
            break;
        case '领取奖励':
            url = `${url}/categoryUnion/getInfo`;
            body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}`
            break;
        case 'luckyDraw':
            url = `${url}/categoryUnion/luckyDraw`;
            body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&actorUuid=${$.actorUuid}`
            break;
        case 'getDrawRecordHasCoupon':
            ...
            break;
        case 'getShareRecord':
            url = `${url}/categoryUnion/getAssistInfo`;
            ...
            case 'activityContent':
                if (typeof res == 'object') {
                    if (res.result && res.result === true) {
                        ...
                        $.taskData = res.data.drawContent || {}
                        $.actorUuid = res.data.actorUuid || ''
                        $.unionShopInfos = res.data.unionShopInfo || []
                        $.allOpenCard = res.data.openCardStatus.data.allOpenCard || false
                        $.openList = res.data.openCardStatus.data.openInfo || []
...
function getPostRequest(url, body, method = "POST") {
    ...
    if (url.indexOf('https://lzdz-isv.isvjcloud.com') > -1) {
        headers["Referer"] = `https://lzdz-isv.isvjcloud.com/categoryUnion/activity?activityId=${$.activityId}&tplId=0003&tplId=0003&shareUuid=${$.shareUuid}`
...
function getCk() {
    return new Promise(resolve => {
        let get = {
            url: `https://lzdz-isv.isvjcloud.com/categoryUnion/activity?activityId=${$.activityId}&tplId=0003&shareUuid=${$.shareUuid}`,
            headers: {
                "User-Agent": $.UA,
            },
            timeout: 30000
