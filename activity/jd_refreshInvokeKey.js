...
let fs = require("fs");
let notify = require('./sendNotify.js')
...
function getHtml() {
    return new Promise(resolve => {
        $.get(getOptions('https://prodev.m.jd.com/mall/active/2tZssTgnQsiUqhmg5ooLSHY9XSeN/index.html'), (err, resp, data) => {
            if (err) {
                $.logErr(err)
            }else {
                resolve(data || '')
...
function getJS(html) {
    ...
    let jsUrl = `https://storage.360buyimg.com/${match(/"htmlSourceUrl":"([^\"]+)"/, html)}`
    return new Promise(resolve => {
        $.get(getOptions(jsUrl), (err, resp, data) => {
            if (err) {
                $.logErr(err)
            }else {
                resolve(data || '')
        ...
        $.get(getOptions(indexUrl), (err, resp, data) => {
            if (err) {
                ...
                if (invokeKey) {
                    ...
                    try {
                        let cc = require("./utils/HConfig.js")
                        cc.invokeKey = invokeKey
                        config = cc
                    }catch(e){}
                    data = `module.exports = ${JSON.stringify(config)}`
                    fs.writeFile('./utils/HConfig.js', data, function(err, data) {
                        if (err) {
