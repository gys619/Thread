...
!(async () => {
  ...
  if(Object.getOwnPropertyNames($.InviteList).length > 0){
    console.log('助力')
    for (let i = 0; i < cookiesArr.length; i++) {
      if (cookiesArr[i]) {
        $.cookie = cookiesArr[i] + '';
...
async function run() {
  try{
    ...
    await GetUserInfo()
    if($.myInfo){
      $.InviteList[$.myInfo.shareId] = []
      await GetAssetsRecord()
      console.log(`等级:${$.myInfo.level}(${$.myInfo.levelComplete}/${$.myInfo.levelTarget}) 累计获得:${$.myInfo.hbSum || 0}元 预估瓜分${$.myInfo.luckyWorth}元 我的年货:${$.myInfo.goodsNum} 助力码:${$.myInfo.shareId}\n`)
      if($.myInfo.drawKey){
        await drawKey()
      }
    }else{
    ...
    if($.myInfo.levelCost <= $.myInfo.goodsNum){
      let goodsNum = $.myInfo.goodsNum
      console.log('升级')
      do{
        await GreetUpgrade()
        await $.wait(parseInt(Math.random() * 2000 + 1000, 10))
        if(goodsNum == $.myInfo.goodsNum) break
        goodsNum = $.myInfo.goodsNum
      }while ($.myInfo.levelCost <= $.myInfo.goodsNum)
      await GetUserInfo()
    }
    await BestWishes()
    if(Object.getOwnPropertyNames($.InviteList).length == 0 || $.InviteList[$.myInfo.shareId].length == 0) delete $.InviteList[$.myInfo.shareId]
    await $.wait(parseInt(Math.random() * 2000 + 4000, 10))
  }
  catch (e) {
    console.log(e);
...
async function GetUserInfo() {
  try{
    let additional= `&strInviteId=&nopopup=0`
    let stk= `nopopup,strInviteId`
    $.HomeInfo = await taskGet(`jxnhj/GetUserInfo`, stk, additional)
    if($.HomeInfo){
      $.myInfo = $.HomeInfo.myInfo || ''
    }
...
async function drawKey() {
  try{
    stk = 'scene,strSceneType'
    additional = `&strSceneType=${$.myInfo.drawKey}&scene=2`
    let res = await taskGet(`jxnhj/Draw`, stk, additional)
    if(res.iRet == 0 && res.data){
      let msg = ''
      for(let i of res.data || []){
        if(msg) msg += `|`
        msg += `${i.prizeName.replace('\u0005','').replace('\u0006','')} ${i.quota}`
      }
      if(msg) console.log("领取弹窗奖励-获得："+msg)
    }else if(res.sErrMsg || res.msg){
      console.log('领取弹窗奖励:'+res.sErrMsg || res.msg)
    }else{
      console.log('领取弹窗奖励\n'+$.toStr(res,res))
    }
  } catch (e) {
...
async function GetTaskList() {
  try{
    let additional= ``
    let stk= ``
    let GetTaskList = await taskGet(`jxnhj/GetTaskList`, stk, additional)
    if(GetTaskList.iRet == 0 && GetTaskList.data && GetTaskList.data.taskList){
      $.task = GetTaskList.data.taskList
      let res = ''
      for(let i of $.task || []){
        console.log(`[${i.taskName}(${i.completedTimes}/${i.targetTimes})] ${i.description || ''} ${ 1 == i.awardStatus ? "已完成" : 2 == i.gettaskStatus ? "未完成 +"+i.reward+"年货" : 0 == i.gettaskStatus ? "领取奖励 +"+i.reward+"年货" : "未知"}`)
        if([14,4].includes(Number(i.taskType))){
          if(i.taskType == 4 && i.targetTimes != i.completedTimes){
            for(let n=0;n<i.targetTimes-i.completedTimes;n++)
            $.InviteList[$.myInfo.shareId] = [...[i.taskId+"-=-"+n]]
            continue
          }
          let dt = i.description.match(/浏览(\d+)秒/) && i.description.match(/浏览(\d+)秒/)[1] || 0
          let flag = 0
          if(dt != 0 && i.gettaskStatus == 2 && i.awardStatus == 2){
            stk = 'bizCode,configExtra,strShareId,taskId'
            additional = `&taskId=${i.taskId}&strShareId=&bizCode=jxnhj_task&configExtra=`
            res = await taskGet(`jxnhj/DoTask`, stk, additional)
            if(res.ret == 0 && res.data){
              let msg = ''
              if(res.data.awardStatus == 2) msg += res.data.awardStatus
              if(msg) flag = 1
              else console.log('任务完成失败\n'+$.toStr(res,res))
            }else if(res.sErrMsg || res.msg){
              console.log('任务领取失败:'+res.sErrMsg || res.msg)
            }else{
              console.log('任务失败\n'+$.toStr(res,res))
            }
            if(flag){
              console.log(`等待${dt}秒`)
              dt = Number(dt)+1
              await $.wait(parseInt(Math.random() * 2000 + dt*1000, 10))
            }
          }
          if((i.gettaskStatus == 0 && i.awardStatus == 2) || flag == 1 || (i.taskType == 4 && i.targetTimes == i.completedTimes && i.awardStatus != 1)){
            stk = 'bizCode,source,taskId'
            additional = `&taskId=${i.taskId}&bizCode=jxnhj_task&source=jxnhj_task`
            res = await taskGet(`newtasksys/newtasksys_front/Award`, stk, additional)
            if(res.ret == 0 && res.data){
              let msg = ''
              if(res.data.prizeInfo){
                res.data.prizeInfo = $.toObj(res.data.prizeInfo,res.data.prizeInfo)
                if(res.data.prizeInfo.ddwAward) msg += `获得${res.data.prizeInfo.ddwAward}年货`
                if(res.data.prizeInfo.ddwExtAward) msg += `|${res.data.prizeInfo.ddwExtAward}`
              }
              if(msg) {
                $.taskFlag = true
                console.log(`${msg}`)
              } else console.log('奖励获取失败\n'+$.toStr(res,res))
            }else if(res.sErrMsg || res.msg){
              console.log('领取奖励失败:'+res.sErrMsg || res.msg)
            }else{
              console.log('领取奖励失败\n'+$.toStr(res,res))
            }
            await $.wait(parseInt(Math.random() * 2000 + 1000, 10))
...
// 奖励记录
async function GetAssetsRecord() {
  try{
    stk = 'dwPageIndex,dwPageSize,dwTabType'
    additional = `&dwPageIndex=1&dwPageSize=20&dwTabType=1`
    let res = await taskGet(`jxnhj/GetAssetsRecord`, stk, additional)
    if(res.iRet == 0 && res.data){
      $.myInfo.hbSum = res.hbSum || 0
    }
...
// 获取助力信息
async function BestWishes() {
  try{
    stk = ''
    additional = ``
    let res = await taskGet(`jxnhj/BestWishes`, stk, additional)
    if(res.iRet == 0 && res.data){
      let msg = []
      if(res.data.shareId){
        for(let i of res.data.pages || []){
          if(i.status == 1) msg.push(i.id)
        }
        if(msg.length > 0) $.InviteList[$.myInfo.shareId] = [...$.InviteList[$.myInfo.shareId],...msg]
      }
    }
  } catch (e) {
    console.log("助力信息\n"+e)
...
async function help(shareId,id){
  ...
  try{
    ...
    if(id > 20){
      stk = 'configExtra,bizCode,strShareId,taskId'
      additional = `&taskId=${id}&strShareId=${shareId}&bizCode=jxnhj_task&configExtra=`
      res = await taskGet(`jxnhj/DoTask`, stk, additional)
    }else{
      stk = ''
      additional = `&id=${id}&shareId=${shareId}`
      res = await taskGet(`jxnhj/WishHelp`, stk, additional)
    }
    if(res.iRet == 0 && res.data){
      let msg = ''
      if(id > 20){
        for(let i of res.data.prizeInfo || []){
          if(msg) msg += `|`
          if(i.awardType != 0) msg += `${i.prizeDesc}${i.prizeType}${i.prizeName}`
        }
      }else{
...
// 升级
async function GreetUpgrade() {
  try{
    stk = ''
    additional = ``
    let res = await taskGet(`jxnhj/GreetUpgrade`, stk, additional)
    if(res.iRet == 0 && res.prizeInfo){
      let msg = ''
      if(res.myInfo){
        $.myInfo.levelCost = res.myInfo.levelCost
        $.myInfo.goodsNum = res.myInfo.goodsNum
      }
      for(let i of res.prizeInfo || []){
        if(msg) msg += `|`
        msg += `${i.prizeName.replace('\u0005','').replace('\u0006','')} ${i.quota}`
      }
      if(msg) console.log("获得："+msg)
    }else if(res.sErrMsg || res.msg){
      console.log('升级失败:'+res.sErrMsg || res.msg)
    }else{
      console.log('升级失败\n'+$.toStr(res,res))
    }
  } catch (e) {
...
function getGetRequest(type, stk='', additional='') {
  ...
  let dwEnv = 4;
  let types = type
  let stks = '&_stk=__t,dwEnv'
  if(stk) stks += `,${stk}`
  if(type.indexOf('/WishHelp') > -1) stks = ''
  url = `https://wq.jd.com/${types}?__t=${Date.now()}&dwEnv=${dwEnv}${additional}${stks}&_ste=1&sceneval=2&g_login_type=1&g_ty=ajax`
  url += `&h5st=undefined`;
  return {
    url,
    headers: {
