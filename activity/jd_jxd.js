...
let shareCodes = ['wddpzLSxORvLGo', 'shy9198', 'jd_692b4f610e1d1', 'jd_4ed37a5bc30e7', 'jd_JfXWSBpPNIgY', 'jd_484e2a39c8430', '被折叠的记忆33', 'jd_6cd93e613b0e5']
              ...
              for (let vo of taskList) {
                if (vo['status'] === 1) {
                  ...
                  for(let i = vo.quantityCompleted; i<vo.quantityToBeCompleted;++i) {
                    console.log(`去完成第【${i+1}/${vo.quantityToBeCompleted}】关注/收藏任务`)
                    await accomplishTask(vo['taskId'])
                    await $.wait(5*1000)
                  }
                  await awardTask(vo['taskId'])
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['returnCode'] === 200) {
              ...
              $.MyWinningInformation = $.MyWinningInformation.filter(item => !!item && !item['prizeName'].includes('京豆'));
