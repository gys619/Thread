...
[task_local]
#5.11-5.18 入会瓜分百万京豆，赢时光好礼
35 2 13-18 5 * jd_opencardL141.js, tag=5.11-5.18 入会瓜分百万京豆，赢时光好礼, enabled=true

*/
const $ = new Env("5.11-5.18 入会瓜分百万京豆，赢时光好礼");
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/baby/pairys/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=undefined&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=1000001683&sid=&un_area=`;
      await member();
      await $.wait(500);
      if ($.bean > 0) {
