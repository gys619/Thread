...
!(async () => {
  ...
  authorCodeList = await getAuthorCodeList('https://raw.githubusercontent.com/11111130/duck/master/code/lzdz3.json')
  if ($.getAuthorCodeListerr === false) {
      authorCodeList = [
        "32e4a24c18044a5b897d6479eb7cb22c",
        "bdaed34f602847d38b368ba5a4118d7b",
        "73a5b91332b44d3ea912d8ed61d154f8"
      ]
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.activityId = "dz42e1a74695424e06a97da4129c64";
      $.activityShopId = "1000088849";
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/jddrink/winecabinet/activity?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=${$.activityShopId}&sid=&un_area=`;
      await member();
      await $.wait(1000);
      if ($.bean > 0) {
...
async function member() {
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      await taskaccessLog("common/accessLogWithAD", `venderId=${$.activityShopId}&code=99&pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&pageUrl=${$.activityUrl}&subType=app&adSource=vice`, 1);
      // await task("wxActionCommon/getUserInfo", `pin=${encodeURIComponent($.secretPin)}`, 1);
      if ($.index === 1) {
        await task("jddrink/winecabinet/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1);
      } else {
        await task("jddrink/winecabinet/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&shareUuid=${encodeURIComponent($.authorCode)}`);
      }
      $.log("关注店铺");
      await task("jddrink/winecabinet/saveTask", `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=6&taskValue=null`);
      await task("taskact/common/drawContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
      await task("jddrink/winecabinet/initOpenCard", `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&actorUuid=${encodeURIComponent($.actorUuid)}`);
      $.log("加入店铺会员");
      if ($.openCardList) {
      ...
      console.log("去助力 -> " + $.authorCode);
      $.log("浏览");
      await task("jddrink/winecabinet/saveTask", `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=12&taskValue=null`);
            ...
            if (data.result) {
              switch (function_id) {
                ...
                case "wxActionCommon/getUserInfo":
                  break;
                case "jddrink/winecabinet/activityContent":
                  if (!data.data.hasEnd) {
                  ...
                  break;
                case "jddrink/winecabinet/initOpenCard":
                  $.openCardList = data.data.openInfo;
                  $.openCardStatus = data.data.allOpenCard;
                  // console.log(data)
                  break;
                case "jddrink/winecabinet/saveTask":
                  console.log(data);
                  break;
                case "jddrink/winecabinet/draw":
                  console.log(data);
                  break;
