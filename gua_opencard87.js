/*
12.21~12.30 联合开卡 [gua_opencard87.js]
新增开卡脚本
一次性脚本
...
2.开14张 成功开1张 可能获得5京豆
  全部开完获得1次抽奖
3.关注10京豆 获得1次抽奖
4.加购5京豆 获得1次抽奖(默认不加购 如需加购请设置环境变量[guaopencard_addSku87]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw87]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw87="3"
填非数字会全都抽奖
...
const $ = new Env('12.21~12.30 联合开卡-87');
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku87 ? process.env.guaopencard_addSku87 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku87') ? $.getdata('guaopencard_addSku87') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard87 ? process.env.guaopencard87 : `${guaopencard}`) : ($.getdata('guaopencard87') ? $.getdata('guaopencard87') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait87 ? process.env.guaopenwait87 : `${guaopenwait}`) : ($.getdata('guaopenwait87') ? $.getdata('guaopenwait87') : `${guaopenwait}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw87 ? process.env.guaopencard_draw87 : guaopencard_draw) : ($.getdata('guaopencard_draw87') ? $.getdata('guaopencard_draw87') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard87]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
...
async function run() {
  try {
    ...
    if(guaopencard_addSku+"" == "true"){
      ...
      console.log('如需加购请设置环境变量[guaopencard_addSku87]为"true"');
    }
    await takePostRequest('activity_load');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw87]为"3" 3为次数');
