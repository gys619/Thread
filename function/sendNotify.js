...
if (config.QQ_SKEY) {
    QQ_SKEY = config.QQ_SKEY;
}
if (config.QQ_MODE) {
    QQ_MODE = config.QQ_MODE;
}
if (config.BARK_PUSH) {
    if (config.BARK_PUSH.indexOf('https') > -1 || config.BARK_PUSH.indexOf('http') > -1) {
        //兼容BARK自建用户
        BARK_PUSH = config.BARK_PUSH
    } else {
        BARK_PUSH = `https://api.day.app/${config.BARK_PUSH}`
    }
...
if (config.TG_USER_ID) {
    TG_USER_ID = config.TG_USER_ID;
}
if (config.TG_PROXY_AUTH) TG_PROXY_AUTH = config.TG_PROXY_AUTH;
if (config.TG_PROXY_HOST) TG_PROXY_HOST = config.TG_PROXY_HOST;
if (config.TG_PROXY_PORT) TG_PROXY_PORT = config.TG_PROXY_PORT;
if (config.TG_API_HOST) TG_API_HOST = config.TG_API_HOST;
...
if (config.QYWX_KEY) {
    QYWX_KEY = config.QYWX_KEY;
}
if (config.QYWX_AM) {
    QYWX_AM = config.QYWX_AM;
}
if (config.IGOT_PUSH_KEY) {
    IGOT_PUSH_KEY = config.IGOT_PUSH_KEY
}
...
function BarkNotify(text, desp, params = {}) {
    return new Promise(resolve => {
        if (BARK_PUSH) {
            const options = {
                url: `${BARK_PUSH}`,
                body: `title=${encodeURIComponent(text)}&body=${encodeURIComponent(desp)}&sound=${BARK_SOUND}&${querystring.stringify(params)}&group=${BARK_GROUP}`,
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                timeout: 10000
