...
async function jdPlantBean() {
    try {
        ...
        if ($.plantBeanIndexResult && $.plantBeanIndexResult.code === '0' && $.plantBeanIndexResult.data) {
            ...
            // 报告运行次数
            $.get({
                    url: `https://cdn.11111112/api/runTimes?activityId=bean&sharecode=${$.myPlantUuid}`,
                    headers: {
                        'Host': 'api.11111117'
                    },
                    timeout: 10000
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({ url: `https://cdn.11111112/api/bean/${randomCount}`, headers: { 'Host': 'api.11111117' }, timeout: 10000 }, (err, resp, data) => {
            try {
                if (err) {
