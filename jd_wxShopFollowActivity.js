...
!(async () => {
    ...
    for (let a in activityIdList) {
        ...
        console.log("活动入口: https://lzkj-isv.isvjd.com/wxShopFollowActivity/activity/activity?activityId=" + activityId)
...
function task(function_id, body, isCommon = 0) {
    if ($.LZ_TOKEN_KEY && $.LZ_TOKEN_VALUE && !cookie.includes('LZ_TOKEN_KEY')) {
                        ...
                        if (data.result) {
                            switch (function_id) {
                                ...
                                case 'activityContentOnly':
                                    ...
                                    $.drawContentVOs = $.activityContent.drawContentVOs;
                                    $.canJoin = $.activityContent.canJoin; // 参与资格
                                    $.hasJoin = $.activityContent.hasJoin; // 参与状态
                                    $.hasFollow = $.activityContent.hasFollow // 关注状态
                                    if ($.index == 1) {
                                        console.log("奖品列表")
                                        for (let i = 0; i < $.drawContentVOs.length; i++) {
                                            console.log('❖ ' + $.drawContentVOs[i]['name'])
                                        }
                                        console.log('');
                                    }
                                    break;
...
async function unfollow() {
    let opt = {
        ...
        body: `functionId=followShop&body={"follow":"false","shopId":"${$.shopId}","award":"true","sourceRpc":"shop_app_home_follow"}&osVersion=13.7&appid=wh5&clientVersion=9.2.0&loginType=2&loginWQBiz=interact`
    }
    return new Promise(resolve => {
        $.post(opt, (err, data) => {
            try {
