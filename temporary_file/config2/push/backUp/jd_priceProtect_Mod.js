...
!(async() => {
	...
	for (let i = 0; i < cookiesArr.length; i++) {
		...
		if (cookiesArr[i]) {
			...
			await main()
			if (strNotify) {
				strAllNotify += `****【京东账号${$.index} ${$.nickName || $.UserName}】****\n` + strNotify+`\n\n`;
			}
		}
	}
	if($.isNode() && strAllNotify){
		await notify.sendNotify(`${$.name}`, strAllNotify);
	}
})().catch((e) => {
	$.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
}).finally(() => {
...
async function main() {
	...
	if (vo) {
		if (vo.result === '0') {
			await skuOnceApply(vo.token);
			if ($.flag) {
				await appliedSuccAmount()
			}
		} else {
			$.log('获取token失败，请重试！')
		}
...
async function appliedSuccAmount() {
	let body = {
		"sid": "",
		"type": "3",
		"forcebot": "",
		...
		$.post(taskUrl("siteppM_appliedSuccAmount", body), (err, resp, data) => {
			try {
				if (err) {
					...
					strNotify = "保价失败：API请求失败，请检查网路重试";
				} else {
					data = JSON.parse(data)
						if (data) {
							if (data.flag) {
								console.log(`保价成功：获得${data.succAmount}元`);
								strNotify = `保价成功：获得${data.succAmount}元`;
							} else {
								console.log("保价失败：没有需要保价的订单.");
								//strNotify = "保价失败：没有需要保价的订单.";
							}
						}
				}
			} catch (e) {
...
async function skuOnceApply(token) {
	let body = {
		"sid": "",
		"type": "3",
		"forcebot": "",
	...
	return new Promise(resolve => {
		$.post(taskUrl("siteppM_skuOnceApply", body), (err, resp, data) => {
			try {
				if (err) {
					console.log(err)
					console.log(`${$.name} API请求失败，请检查网路重试`)
					...
					data = JSON.parse(data)
						if (data) {
							if (data.flag) {
								...
								strNotify = `保价失败：${data.responseMessage}`;
							}
						}
				}
			} catch (e) {
...
function taskUrl(function_id, body) {
	return {
		url: `https://api.m.jd.com/api?appid=siteppM&functionId=${function_id}&forcebot=&t=${new Date().getTime()}`,
