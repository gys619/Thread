...
(默认不加购 如需加购请设置环境变量[opencardHF_addSku]为"true"
...
cron "35 6,20,22 * * *" script-path=jd_opencardHF.js, tag=健康、食品联合开卡
————————————————
...
opencard_addSku = $.isNode() ? (process.env.opencardHF_addSku ? process.env.opencardHF_addSku : `${opencard_addSku}`) : ($.getdata('opencardHF_addSku') ? $.getdata('opencardHF_addSku') : `${opencard_addSku}`);
    ...
    for(let i in config){
      ...
      if($.task.dailyTask && $.task.dailyTask.taskList){
        for (let t in $.task.dailyTask.taskList) {
          ...
          if(opencard_addSku+"" != "true" && 2 == $.oneTask.groupType) console.log('如需加购请设置环境变量[opencardHF_addSku]为"true"\n');
