...
(默认加购 如不需加购请设置环境变量[opencardFood_addSku]为"false"
...
30 6,17 * 9 * jd_opencardFood.js, tag=会员“食”力派, enabled=true
...
if ($.isNode() && process.env.opencardFood_addSku) {
    opencard_addSku = $.isNode() ? (process.env.opencardFood_addSku ? process.env.opencardFood_addSku : `${opencard_addSku}`) : ($.getdata('opencardFood_addSku') ? $.getdata('opencardFood_addSku') : `${opencard_addSku}`);
}
    ...
    for(let i in config){
      ...
      if($.task.dailyTask && $.task.dailyTask.taskList){
        for (let t in $.task.dailyTask.taskList) {
          ...
          if(opencard_addSku+"" != "true" && 2 == $.oneTask.groupType) console.log('如需加购请设置环境变量[opencardFood_addSku]为"true"\n');
          if(opencard_addSku+"" != "true" && 2 == $.oneTask.groupType) continue;
