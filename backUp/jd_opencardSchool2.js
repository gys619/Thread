/*
8.28-9.10 奔跑吧 开学季2[jd_opencardSchool2.js]
新增开卡脚本
一次性脚本
...
————————————————
入口：[8.28-9.10 奔跑吧 开学季2 (https://lzdz-isv.isvjcloud.com/dingzhi/union/kxj/activity/8729993?activityId=902082701&shareUuid=d26dc254d4af45fcbe39820134d9e25d)]

============Quantumultx===============
[task_local]
#8.28-9.10 奔跑吧 开学季2
34 2,14 3-10 9 * https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_opencardSchool2.js, tag=9.3-9.13 奔跑吧 开学季, enabled=true

================Loon==============
[Script]
cron "34 2,14 3-10 9 *" script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_opencardSchool2.js, tag=9.3-9.13 奔跑吧 开学季

===============Surge=================
8.28-9.10 奔跑吧 开学季2 = type=cron,cronexp="34 2,14 3-10 9 *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_opencardSchool2.js

============小火箭=========
8.28-9.10 奔跑吧 开学季2 = type=cron,script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_opencardSchool2.js, cronexpr="34 2,14 3-10 9 *", timeout=3600, enable=true
*/
...
let opencardSchool2_addSku = "true"
opencardSchool2_addSku = $.isNode() ? (process.env.opencardSchool2_addSku22 ? process.env.opencardSchool2_addSku22 : opencardSchool2_addSku) : ($.getdata('opencardSchool2_addSku22') ? $.getdata('opencardSchool2_addSku22') : opencardSchool2_addSku);
let opencard_draw = "40"
opencard_draw = $.isNode() ? (process.env.opencardSchool2_draw ? process.env.opencardSchool2_draw : opencard_draw) : ($.getdata('opencardSchool2_draw') ? $.getdata('opencardSchool2_draw') : opencard_draw);
let opencard = "true"
opencard = $.isNode() ? (process.env.opencardSchool2 ? process.env.opencardSchool2 : opencard) : ($.getdata('opencardSchool2') ? $.getdata('opencardSchool2') : opencard);
message = ""
!(async () => {
    if (!cookiesArr[0]) {
    ...
    if ($.isNode()) {
        if(opencard+"" != "true"){
            console.log('如需执行脚本请设置环境变量[opencardSchool2]为"true"')
        }
        if(opencard+"" != "true"){
            return
        }
    }
    $.shareUuid = 'd26dc254d4af45fcbe39820134d9e25d'
...
async function run(){
    try{
        ...
        if(opencardSchool2_addSku+"" == "true"){
            if(!$.add2cart) await saveTask('加购商品', "add2cart&param=");
        }else console.log('如需加购请设置环境变量[opencardSchool2_addSku22]为"true"');
        ...
        }else console.log('如需抽奖请设置环境变量[opencardSchool2_draw]为"true"');
        if(gameFlag == 1){
            await $.wait(parseInt(Math.random() * 1000 + 2000, 10))
            await getActorUuid()
            await $.wait(parseInt(Math.random() * 1000 + 2000, 10))
