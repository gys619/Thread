...
/*
9.18~9.28 月满金秋 佳节聚惠 [jd_opencardYJ.js]
一次性开卡脚本
邀请一人20豆 被邀请也有20豆(有可能没有豆
开卡 抽奖有机会获得30京豆或实物(有可能有抽到空气💨
加购5京豆 (有可能是空气💨
第一个账号助力作者 其他依次助力CK1
第一个CK失效会退出脚本
————————————————
入口：[9.18~9.28 月满金秋 佳节聚惠 (https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/7681574?activityId=21h3i1h3h2u1h3g43gy4g3yu2u23u423&shareUuid=c00bbce7407e41dbb18408226667caa7)]
IOS等用户直接用NobyDa的jd cookie
============Quantumultx===============
[task_local]
#9.18~9.28 月满金秋 佳节聚惠
20 5,20 18-28 9 * https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_opencardYJ.js, tag=9.18~9.28 月满金秋 佳节聚惠, enabled=true
================Loon==============
[Script]
cron "20 5,20 18-28 9 *" script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_opencardYJ.js,tag=9.18~9.28 月满金秋 佳节聚惠
===============Surge=================
9.18~9.28 月满金秋 佳节聚惠 = type=cron,cronexp="20 5,20 18-28 9 *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_opencardYJ.js
============小火箭=========
9.18~9.28 月满金秋 佳节聚惠 = type=cron,script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_opencardYJ.js, cronexpr="20 5,20 18-28 9 *", timeout=3600, enable=true
*/

const $ = new Env('月满金秋 佳节聚惠');
...
!(async () => {
    ...
    console.log(`入口：\nhttps://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/7681574?activityId=21h3i1h3h2u1h3g43gy4g3yu2u23u423&shareUuid=c00bbce7407e41dbb18408226667caa7`);
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/${random(1000000, 9999999)}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await marry();
            await $.wait(3000)
            if ($.bean > 0) {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.index === 1) {
                ...
                await task('dz/openCard/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${$.attrTouXiang}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1)
            } else {
                await task('dz/openCard/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${$.attrTouXiang}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`)
            }
            ...
            try {
                if (err) {
                    ...
                    if (data) {
                        ...
                        if (data.result) {
                            if($.index==1) $.authorCode = 'c00bbce7407e41dbb18408226667caa7';
                            switch (function_id) {
