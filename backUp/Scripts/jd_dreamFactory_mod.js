...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN('https://cdn.jsdelivr.net/gh/TongLin138/Test@main/utils/empty.json')
  if (!res) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/TongLin138/Test@main/utils/empty.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await updateTuanIdsCDN('https://cdn.jsdelivr.net/gh/TongLin138/Test@main/utils/empty.json');
  }
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://cdn.jsdelivr.net/gh/TongLin138/Test@main/backUp/Code/dreamFactory.json`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
          console.log(`${$.name} API请求失败，请检查网路重试`)
