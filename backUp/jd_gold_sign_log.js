...
[task_local]
#京东金榜
9 7 * * * jd_gold_sign.js, tag=京东金榜, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

 */
const $ = new Env('京东金榜-LOG版');
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === '0') {
                await goldCreatorDoTask({ "type": 1 , "random":random ,"log":log ,"sceneid":"jdgoldenranking2022h5"  })
            } else {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === '0') {
              if (data.result.medalNum === 5) {
                await $.wait(1500)
                await goldCreatorDoTask({ "type": 2 , "random":random ,"log":log ,"sceneid":"jdgoldenranking2022h5"  })
              }
            } else {
              console.log(`失败：${JSON.stringify(data)}\n`);
            }
