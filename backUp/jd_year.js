...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if ($.token) {
            ...
            if ($.hotFlag === false) { console.log('活动火爆啦，快去买买买吧'); continue }
            if ($.accessToken) {
...
async function main() {
    ...
    if ($.token) {
        await taskPost('jd-user-info', `token=${$.token}&source=01`);
        if ($.hotFlag === false) { console.log('活动火爆啦，快去买买买吧'); return }
        if ($.accessToken) {
            await task('get_user_info', `uuid=${$.uuid}&source=shareFriend`);
            $.t1 = h();
            $.t2 = y();
            ...
            if ($.tasklist) {
                ...
                if ($.index === 1) {
                    console.log('CK1助力作者')
                    await taskPost('invite_friend', `inviter_id=61e60fb4a3b7750068&uuid=${$.uuid}&source=shareFriend`);
                }
            }
